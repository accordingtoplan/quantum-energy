...
- product.html: PDP direction cut. Sticky plain nav, Block 1 stage (gallery with thumbs, purchase panel with flavor chips that take over tint and accent, one-time vs subscribe, quantity, add to cart, badges), Block 0 sticky buy bar on mobile, Block 2 argument, Block 3 curve (same module), Block 4 three numbers as cards, Block 12 close plus cross-sell tiles and footer. Blocks 5 to 9 not built yet. Four flavors carry chips (the ones with packshots); salted PB crunch and caramel almond descriptions are placeholder copy. Subscription price is 10% off as a placeholder until Katie Lee.
...
1. Frederik reviews product.html live, then PDP Blocks 5 (ingredient flex), 6 (moment selector tabs), 8 (comparison with wit) and static 9 (social proof) get built.
